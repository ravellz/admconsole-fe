...
const WelcomeMessage = () => {
  ...
  const [welcomeMessageChannels, setWelcomeMessageChannels] = useState([]);
  ...
  useEffect(() => {
    ...
    const welcomeMessageChannelsData = dataChannels && dataChannels.welcomeMessageChannels ? dataChannels.welcomeMessageChannels : [];
    setWelcomeMessageChannels(welcomeMessageChannelsData);
  }, []);
  ...
  const saveChannelsToServer = async (enabledValue) => {
    ...
    const channelsToSave = selectedChannelsToSave.map(channelId => {
      const selectedChannel = discordData.channels.find(channel => channel.id === channelId);
      return {
    ...
    await axios.post(`${API_BASE_URL}/saveWelcomeMessageChannels`, { welcomeMessageChannels: channelsToSave });
  };
  ...
  const handleSelectChannel = (e) => {
    ...
    if (!selectedChannelsToSave.includes(channelId)) {
      ...
      setSelectedChannelsToSave(selectedChannelsToSave.filter(id => id !== channelId));
    }
  };
    ...
    <div className="container-log">
      <div className="box has-text-centered">
        <div className="column">
          <label className="label">Welcome Log</label>
          <div className="field">
            <button
              className={`button is-small ${enabled ? "is-primary" : "is-danger"}`}
            ...
            <button
              className={`button is-small ${!enabled ? "is-primary" : "is-danger"} ${isSaving ? "is-loading" : ""}`}
          ...
          <div className="field">
            <div className="box">
              <div className="field">
                ...
                <ul>
                {welcomeMessageChannels.map((channel, index) => (
                <li key={index}>
